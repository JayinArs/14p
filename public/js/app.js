...
window.appGalleryEditors = [];
(function($, window, document, undefined) {
    // Create the defaults once
    var appGalleryEditorName = 'appGalleryEditor',
        defaults = {
            'viewer': 'view',
            'adder': 'add',
    ...
    function appGalleryEditor(element, options) {
        ...
        this._name = appGalleryEditorName;
        window.appGalleryEditors.push(this);
        this._index = window.appGalleryEditors.length - 1;
        this.init();
    }

    appGalleryEditor.prototype.init = function() {
        var _this = this;
        ...
        _this.$adder.on('click', '[data-action="add"]', function(e) {
            var _self = this;
            localStorage.setItem('target_action', 'appGalleryEditor');
    ...
    appGalleryEditor.prototype.add = function(data) {
        var _this = this;
    ...
    appGalleryEditor.prototype.remove = function(index) {
        $(this.$viewer.find('.gallery-item')[index]).remove();
    }

    appGalleryEditor.prototype.serialize = function() {
        var _this = this;
        var ret = [];
    ...
    $.fn.appGalleryEditor = function(options) {
        var _this = this;
        var retval = this;
        _this.each(function() {
            var plugin = $(this).data(appGalleryEditorName);
            if (!$(this).data(appGalleryEditorName)) {
                $(this).data(appGalleryEditorName, new appGalleryEditor(this, options));
            } else {
...
function SetUrl(url) {
    ...
    } else if (target_action == 'appGalleryEditor') {
        var target_gallery_index = localStorage.getItem('target_index');
        console.log(target_gallery_index);
        var gallery = window.appGalleryEditors[target_gallery_index];
        gallery.add({
            image: url
        });
    }
...
(function(window, document, $, undefined) {
    ...
    $(function() {
        ...
        $("#app-settings input[name='theme']").on('change', function() {
            var $this = $(this);
            $.ajax({
                url: window.params.url + 'dashboard/user_setting',
                type: 'POST',
                data: {
                    'theme': $this.val()
                }
        ...
        $("#app-settings input[type='checkbox']").on('change', function() {
            ...
            $.ajax({
                url: window.params.url + 'profile/setting',
                type: 'POST',
                data: data
            });
        });
